...
public:
    ...
    // Подробное пояснение смотрите в README.md
    void HandleBeginFrame(StringHash eventType, VariantMap& eventData)
    {
        ...
        if (GLOBAL->neededGameState_ == GS_DEAD)
        {
            ...
            Node* ufoNode = scene_->GetChild("Ufo");
            SoundSource* soundSource = ufoNode->GetOrCreateComponent<SoundSource>();
        ...
        else if (GLOBAL->neededGameState_ == GS_INTRO)
        {
            ...
            UfoLogic* ufoLogic = ufoNode->GetComponent<UfoLogic>();
            ufoLogic->Reset();
            ...
            {
                Vector3 pos = barrier->GetPosition();
                pos.y_ = BAR_RANDOM_Y;
    ...
    void CreateUI()
    {
        ...
        helpText->SetText("Жмите левую кнопку мыши");
    }
    ...
    void CreateUfo()
    {
        Node* ufoNode = scene_->CreateChild("Ufo");
        AnimatedModel* ufoObject = ufoNode->CreateComponent<AnimatedModel>();
        ...
        AnimationController* animCtrl = ufoNode->CreateComponent<AnimationController>();
        animCtrl->PlayExclusive("Models/Fly.ani", 0, true);

        RigidBody* body = ufoNode->CreateComponent<RigidBody>();
        ...
        // Корпус
        CollisionShape* shape1 = ufoNode->CreateComponent<CollisionShape>();
        ...
        // Кабина
        CollisionShape* shape2 = ufoNode->CreateComponent<CollisionShape>();
        ...
        Node* lightNode = ufoNode->CreateChild();
